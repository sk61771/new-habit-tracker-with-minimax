...
export default function IncomeDonutChart({ data, total }) {
  const chartData = Object.entries(data)
    .filter(([_, value]) => value > 0)
    .map(([id, value]) => ({
      name: getIncomeCategoryById(id).name,
      value,
      color: getIncomeCategoryById(id).color
    }));
    ...
    return (
      <div className="h-64 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">No income data</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-64">
      ...
      <div className="flex flex-wrap justify-center gap-3 mt-2">
        {INCOME_CATEGORIES.filter(cat => data[cat.id] > 0).map((cat) => (
